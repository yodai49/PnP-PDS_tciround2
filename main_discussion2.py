...
def test_all_images (experimental_settings_arg = {}, method_arg = {}, configs_arg = {}):
    ...
    for path_img in path_images:
        ...
        # Run evaluation
        # =====================================
        img_sol, s_sol, c_evolution, psnr_evolution, ssim_evolution, average_time, other_data = iteration.test_iter(x_0, img_obsrv, img_true, phi, adj_phi, gamma1, gamma2, alpha_s, alpha_n, myLambda, m1, m2, gammaInADMMStep1, lambydaInStep2, gaussian_nl, sp_nl, poisson_alpha, path_prox, max_iter, method, ch, r)
        if(poisson_noise):
        ...
        path_saveimg_base = method + '_' + architecture + '_' + deg_op + '_' + str(gaussian_nl).ljust(5, '0') + '_' + str(poisson_alpha) + '_(' + filename + ')' + '_alpha' + str(alpha_n).ljust(5, '0') + '_lambda' + str(myLambda).ljust(5, '0') + '_gamma1' + str(gamma1).ljust(7, '0')
        if (add_timestamp):
...
def main():
    ...
    noise_level_list = [100]
    obs_list = ['blur', 'random_sampling']
    method_list_G = ['C-Proposed']
    myLambda_list = [0.00125, 0.0005]
    ...
    for nl in noise_level_list:
        for obs in obs_list:
            for architecture in architecture_list:
                ...
                settings =  {'gaussian_nl' : 0, 'sp_nl' : 0, 'poisson_noise' : True, 'poisson_alpha' : nl, 'deg_op' : obs, 'r' : rs}
                ...
                for method_G in method_list_G:
                    for i in range(0,10):
                        ...
                        # REDは0～0.001まで
                        # Unstableは0～10まで
                        if (method_G == 'C-Proposed'):
                            myLambda = myLambda_list[obs_list.index(obs)]
                            #gamma1 = np.sqrt(nl_den_list[architecture_list.index(architecture)])
                            gamma1 = nl_den_list[architecture_list.index(architecture)]
                            gamma1 = gamma1 * (i + 1) * 25
                            gamma2 = 1 / (gamma1 * 2) - 1e-4
                        experiment_data_list.append ({'settings' : settings, 'method' : {'method' : method_G, 'architecture' : architecture, 'max_iter' : max_iter, 'gamma1' :  gamma1, 'gamma2' : gamma2, 'myLambda' : myLambda, 'm1' : 100, 'm2' : 5, 'gammaInADMMStep1' : 1}, 'configs' : configs})
